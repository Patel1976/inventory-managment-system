...
const PurchaseReport = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) { case 'Paid': return 'badge-success'; case 'Partial': return 'badge-warning'; case 'Unpaid': return 'badge-danger'; default: return 'badge-secondary'; }
  };
  ...
  return (
    <div className="purchase-report-page">
      <div className="page-header">
        ...
        <div className="breadcrumb-wrapper"><Link to="/">Home</Link><span>/</span><Link to="/reports/purchase">Reports</Link><span>/</span><span>Purchase Report</span></div>
      </div>
      ...
      <div className="row g-4 mb-4">
        <div className="col-12 col-md-3"><div className="stat-card"><div className="stat-content"><h3>${totals.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</h3><p>Total Purchases</p></div></div></div>
        ...
        <div className="col-12 col-md-3"><div className="stat-card"><div className="stat-content"><h3>{filteredData.length}</h3><p>Total Orders</p></div></div></div>
      </div>
        ...
        <div className="data-card-body">
          <div className="table-responsive">
            <table className="data-table">
              <thead><tr><th>Date</th><th>Reference</th><th>Supplier Name</th><th>Total Amount</th><th>Paid Amount</th><th>Due Amount</th><th>Payment Status</th></tr></thead>
              <tbody>
                {paginatedData.map((purchase, index) => (
                  <tr key={index}>
                    <td>{purchase.date}</td><td><strong>{purchase.reference}</strong></td><td>{purchase.supplier}</td>
                    <td><strong>${purchase.total.toFixed(2)}</strong></td><td style={{ color: '#16a34a' }}>${purchase.paid.toFixed(2)}</td>
                    <td style={{ color: purchase.due > 0 ? '#dc2626' : 'inherit' }}>${purchase.due.toFixed(2)}</td>
                    <td><span className={`badge ${getStatusBadge(purchase.status)}`}>{purchase.status}</span></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: '#f8f9fa', fontWeight: '600' }}>
                  <td colSpan={3}>Total</td><td>${totals.total.toFixed(2)}</td><td style={{ color: '#16a34a' }}>${totals.paid.toFixed(2)}</td><td style={{ color: '#dc2626' }}>${totals.due.toFixed(2)}</td><td></td>
                </tr>
              </tfoot>
            </table>
          </div>
