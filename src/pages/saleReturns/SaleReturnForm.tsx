...
interface Sale { id: number; reference: string; customer?: Customer; }
...
const SaleReturnForm = () => {
  ...
  useEffect(() => {
    if (isEdit && editData) {
      const saleId = String(editData.sale_id || editData.sale?.id || '');
      setFormData({
        sale_id: saleId,
        product_id: String(editData.product_id || editData.product?.id || ''),
      ...
      setCustomerName(editData.sale?.customer?.name || '');
    }
  }, [isEdit, editData]);

  // Update customer when sales list loads in edit mode
  useEffect(() => {
    if (isEdit && formData.sale_id && sales.length > 0 && !customerName) {
      const sale = sales.find(s => String(s.id) === formData.sale_id);
      setCustomerName(sale?.customer?.name || '');
    }
  }, [sales, isEdit, formData.sale_id]);
          ...
          <div className="row g-3">
            <div className="col-12 col-md-6">
              ...
              <select className="form-select" value={formData.sale_id} onChange={e => handleSaleChange(e.target.value)} required>
                ...
                {sales.map(s => <option key={s.id} value={s.id}>{s.reference}{s.customer ? ` - ${s.customer.name}` : ''}</option>)}
              </select>
            </div>

            <div className="col-12 col-md-6">
              <label className="form-label">Customer</label>
              <input type="text" className="form-control" value={customerName} readOnly placeholder="Auto-filled from sale" />
            </div>
